# VPC with public subnets (ALB) and private subnets (EKS worker nodes).
# EKS best practice: nodes/pods in private subnets, load balancer in public.
...
# One NAT gateway per AZ - no cross-AZ single point of failure for egress
